...
export default async function extractTextFromFile({
  ...
  switch (filetype) {
    ...
    case "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet": // i.e. xlsx file
      const xlsxResult = await mammoth.extractRawText({ path: filepath });
      return xlsxResult.value;
    case "text/markdown":
    case "text/csv":
    case "text/html":
